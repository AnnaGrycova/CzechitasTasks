...
it was the grayish white of old porridge.";
			...
			{
				if (string.Compare(blabol1, blabol3) == -1)
				{
					prvni = blabol1;
				}
				else
			...
			else if (string.Compare(blabol3, blabol2) == -1)
			{
				if (string.Compare(blabol3, blabol1) == -1)
				{
					prvni = blabol3;
				}
			}
			...
			string[] rozdelenyText = text.Split(new char[] { '!', '"' }, StringSplitOptions.RemoveEmptyEntries);
			string veta = rozdelenyText[0];
